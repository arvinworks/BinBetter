$(document).ready(function () {
    ...
    let headersGarbage = ['Address', 'Photo', 'Video', 'Description', 'Status', 'Action'];
    headersGarbage.forEach(header => {
        $(`#${tableGarbageId}-headers`).append(`<th class="px-5">${header}</th>`);
    });

    let headersRecycled = ['Address', 'Photo', 'Video', 'Description', 'Status', 'Action'];
    headersRecycled.forEach(header => {
        $(`#${tableRecycledId}-headers`).append(`<th class="px-5">${header}</th>`);
    });
                ...
                render: function (data, type, row) {
                    if (data === null) {
                        ...
                        if (isYouTubeURL(data)) {
                            ...
                            return '<iframe class="datatable-video" width="150" height="100" src="https://www.youtube.com/embed/' + videoId + '" frameborder="0" allowfullscreen></iframe>';
                        } else {
                            return '<video class="datatable-video" width="150" height="100" controls>' +
    ...
    $(document).on('click', '.status-btn', function () {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: `This post will be ${ type === 'reject' ? 'rejected' : 'accepted' }.`,
            icon: 'info',
            showCancelButton: true,
            confirmButtonColor: '#000',
            cancelButtonColor: '#d33',
            confirmButtonText: `Yes, ${ type === 'reject' ? 'reject' : 'accept' } it!`,
            allowOutsideClick: false,
            allowEscapeKey: false,
        }).then((result) => {
            if (result.isConfirmed) {
    ...
    if ($('#type').val() === 'Garbage') {
        ...
        if (photoReports.length === 0) {
            $('#photo_error').html('<strong>Please select at least one photo.</strong>');
            ...
            for (let i = 0; i < photoReports.length; i++) {
                ...
                if (!fileType.match('image.*')) {
                    $('#photo_error').html('<strong>Only image files are allowed (jpeg, png, jpg, webp).</strong>');
                    $('#photo').addClass('border-danger is-invalid');
                    isValid = false;
                ...
                if (fileSize > 2 * 1024 * 1024) {
                    $('#photo_error').html('<strong>Each photo must be less than 2MB.</strong>');
                    $('#photo').addClass('border-danger is-invalid');
                    isValid = false;
                    break;
                }
        ...
        // Validate if files exist
        if (photoReports.length === 0) {
            $('#re_photo_error').html('<strong>Please select at least one photo.</strong>');
            ...
            for (let i = 0; i < photoReports.length; i++) {
                ...
                // Check if the file is an image
                if (!fileType.match('image.*')) {
                    $('#re_photo_error').html('<strong>Only image files are allowed (jpeg, png, jpg, webp).</strong>');
                    $('#re_photo').addClass('border-danger is-invalid');
                    isValid = false;
                    break;  // Stop on the first invalid file
                }
                ...
                // Check file size (e.g., max 2MB)
                if (fileSize > 2 * 1024 * 1024) {
                    $('#re_photo_error').html('<strong>Each photo must be less than 2MB.</strong>');
                    $('#re_photo').addClass('border-danger is-invalid');
                    isValid = false;
                    break;
                }
