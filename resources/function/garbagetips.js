$(document).ready(function () {
                ...
                render: function (data, type, row) {
                    if (data === null) {
                        ...
                        if (isYouTubeURL(data)) {
                            ...
                            return '<iframe class="datatable-video" width="300" height="240" src="https://www.youtube.com/embed/' + videoId + '" frameborder="0" allowfullscreen></iframe>';
                        } else {
                            return '<video class="datatable-video" width="300" height="240" controls>' +
    ...
    $(document).on('click', '.edit-btn', function () {
        ...
        if (photos) {
            ...
            $('#garbageTipModal').find('.photos-container').html('<p>No existing photos.</p>');
        }
        ...
        $('#garbagetip-form').append('<input type="hidden" name="_method" value="PUT">');
            ...
            success: function (response) {
                ...
                if (tutorials.length > 0) {
                    tutorials.forEach(function (tutorial) {
                        const tutorialTimeAgo = dayjs(tutorial.created_at).fromNow();


                        // Filter top-level comments (comments without a parent)
                        const topLevelComments = tutorial.post_garbagetip_comments.filter(comment => !comment.parent_id);
                        ...
                        $('.garbage-tip-list').append(`
                            <div class="tutorial-item" data-tutorialid="${tutorial.id}">
                                ...
                                <button class="btn btn-danger mb-2 rounded-pill report-content" data-tutorialid="${tutorial.id}">Report</button>
                                </div>
                                    <div class="col-xxl-6">
                                    ...
                                    <div class="row">
                                        <div class="col-xxl-3">
                                            <!-- Main Image -->
                                            <img src="${tutorial.photos.split(',')[0]}" style="object-fit:cover;width:100%;height:auto;" alt="Main Image">

                                            <!-- Additional Images -->
                                            <div class="row mt-3">
                                                ${tutorial.photos.split(',').slice(1).map(photo => `
                                ...
                                ? `<iframe class="datatable-video" style="object-fit:cover;width:100%;height:100%;" src="https://www.youtube.com/embed/${extractYouTubeVideoId(tutorial.video)}" frameborder="0" allowfullscreen></iframe>`
                                                ...
                                                <div class="d-flex comment">
                                                    <div class="flex-grow-1 ms-3">
                                                        <div class="form-floating comment-compose mb-2">
                                                            <textarea class="form-control w-100 new-comment-${tutorial.id}" placeholder="Leave a comment here" style="height:2rem;"></textarea>
                                                            <label for="new-comment">Leave a comment here</label>
                                                        </div>
                                                        <div class="hstack justify-content-end gap-1">
                                                            <button class="btn btn-sm btn-secondary rounded-pill cancel-comment" data-tutorialid="${tutorial.id}">Cancel</button>
                                                            <button class="btn btn-sm btn-primary rounded-pill tutorial-comment" data-tutorialid="${tutorial.id}">Comment</button> 
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            
                                            <div class="comment-list vstack gap-4" id="comment-list-${tutorial.id}"></div> <!-- Unique comment list for each tutorial -->
                                        </div>
                        ...
                        const commentContainer = $(`#comment-list-${tutorial.id}`);

                        tutorial.post_garbagetip_comments.forEach(function (comment) {
                            ...
                            if (!comment.parent_id) { // Only display top-level comments
                                ...
                                if (Array.isArray(comment.replies_garbagetip) && comment.replies_garbagetip.length > 0) {
                                    comment.replies_garbagetip.forEach(function (reply) {
                                        const replyTimeAgo = dayjs(reply.created_at).fromNow();
                                        repliesHtml += `
                                            <div class="d-flex reply">
                                                <img class="rounded-circle comment-img" src="${reply.user.profile == null ? 'assets/back/images/avatar/noprofile.webp' : reply.resident.profile}" width="128" height="128">
                                                <div class="flex-grow-1 ms-3">
                                                    <div class="mb-1">
                                                        <a href="#" class="fw-bold link-body-emphasis pe-1">${reply.user?.username || 'Unknown'}</a>
                                ...
                                commentContainer.append(`
                                    <div class="comment-box">
                                        <div class="d-flex comment">
                                            <img class="rounded-circle comment-img" src="${comment.user && comment.user.profile ? comment.user.profile : 'assets/back/images/avatar/noprofile.webp'}" width="128" height="128">
                                            <div class="flex-grow-1 ms-3">
                                                <div class="mb-1">
                                                    <a href="#" class="fw-bold link-body-emphasis me-1">${comment.user ? comment.user.username : 'Unknown'}</a>
                                                ...
                                                <div style="margin-left:-.769rem;">
                                                    <button class="btn btn-primary rounded-pill d-inline-flex align-items-center" data-bs-toggle="collapse" data-bs-target="#collapse-comment${comment.id}" aria-expanded="false" aria-controls="collapse-comment${comment.id}">
                                                        ...
                                                        <span>${comment.replies_garbagetip ? comment.replies_garbagetip.length : 0} replies</span>
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            $('.reply-btn').on('click', function () {
                                ...
                                const tutorialId = $(this).closest('.tutorial-item').data('tutorialid');
                    ...
                    $('.tutorial-comment').on('click', function () {
                        const tutorialID = $(this).attr('data-tutorialid');
                        const replyToID = $(this).attr('data-replyid');
                        const comment = $(`.new-comment-${tutorialID}`).val();
                        sendComment(tutorialID, replyToID, comment)
                    });

                    $('.cancel-comment').on('click', function () {
                        const tutorialID = $(this).attr('data-tutorialid');
                        $(this).closest('.comment-box').find(`.new-comment-${tutorialID}`).val('');
                    });
                    ...
                    $('.report-content').on('click', function () {
                        ...
                        $('#reportContentModal .modal-title').text('Report Content');
                        $('#reportcontent-form').attr('action', '/report-garbagetip');
    ...
    function showReplyForm(commentId, tutorialId) {
        ...
        $(`.tutorial-comment[data-tutorialid="${tutorialId}"]`).attr('data-replyid', commentId);
    }
